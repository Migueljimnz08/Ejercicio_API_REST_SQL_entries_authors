...
UPDATE entries
	SET title='Estamos de jueves', content='Back is back', id_author=(SELECT id_author FROM authors WHERE email='alejandru@thebridgeschool.es'),  category='Software'
	WHERE title='Estamos de Lunes de Back';
-- Busca a todos los autores 
SELECT * FROM authors;
-- Busca a un autor por email
SELECT * FROM authors
    WHERE email='alejandru@thebridgeschool.es';
-- Actualiza los datos de un autor
UPDATE authors
    SET name='Alex', surname='Reinote', email='alejandruelmaschulo@thebridgeschool.es', image='https://randomuser.me/api/portraits/thumb/men/75.jpg'
    WHERE email='alejandru@thebridgeschool.es';
-- Borra una entrada 
DELETE FROM entries
    WHERE title='Estamos de Lunes de Back';
-- Borra un autor 
DELETE FROM authors
    WHERE email='alejandruelmaschulo@thebridgeschool.es';